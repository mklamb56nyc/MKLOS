...
- Molly's birthday Wed Oct 21; Halloween is a Saturday; Toby's EA/ED college
  deadlines land ~Nov 1 (see `toby-college/`).
- Toby (17) and Nate (13) are home in NYC and need coverage while we're away;
  Molly is at boarding school.

## Conventions
- Edit `PLAN.md` in place as decisions land; move superseded options to a
  "ruled out" section rather than deleting them (preserve the reasoning).
- Calendar facts above were read on 2026-07-23 — re-verify against Google
  Calendar before booking anything.
- School dates (Miss Porter's Family Weekend, Poly Prep days off) are **not yet
  confirmed** — they're the biggest unknowns; don't treat them as settled.
- Once dates are chosen: hold them on "Mike and Carrie's Calendar", then track
  bookings (lodging, kid coverage) in `PLAN.md`.
